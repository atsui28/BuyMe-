...
<body>
	...
	try {
		...
		String sql = "insert into autobid(userid, maxprice, itemid) VALUES (?, ?, ?)";
		PreparedStatement ps = con.prepareStatement(sql);
		ps.setString(1, userid);
		ps.setFloat(2, p);
		ps.setString(3, itemid);
		ps.executeUpdate();
		sql = "select TIMESTAMPDIFF(second, NOW(), item.closetime) AS time, b.price, item.bidincrement FROM item JOIN (SELECT itemid, max(price) AS price FROM bids GROUP BY itemid) AS b ON item.itemid = b.itemid WHERE item.itemid = ?";
		...
		if(b>10&&p>=np+bi){
			...
			if(res.next()){
				...
				String userid2 = res.getString("autobid.userid");
				if(p+bi<=ap){
					sql = "insert into bids(userid, price, itemid) VALUES (?, ?, ?)";
